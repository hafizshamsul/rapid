<?php
    ...
    elseif($postjson['action'] == 'authenticate'){
        $query = mysqli_query($mysqli, "select * from users where username='$postjson[username]' and passwordhash='$postjson[password]' and status='Active'");
    ...
    elseif($postjson['action']=='getroompost'){
        ...
        while($row = mysqli_fetch_array($query)){
            ...
            $data[] = array(
                ...
                'post_displayname' => $row['displayname'],
                'dateuploaded' => $ago,
                'vote' => $vote
            );
        }
...
$query = mysqli_query($mysqli, "insert into folderfile set name = '$postjson[name]', icon = 'folder', folder_id = '$postjson[parent]', users_id = 1");
    ...
    elseif($postjson['action']=='addroomusers'){
        $data = array();
        $query = mysqli_query($mysqli, "INSERT INTO roomusers(users_id, room_id, role) values ('$postjson[users_id]', '$postjson[room_id]', 'member')");
        //$id_addroom = mysqli_insert_id($mysqli);
